@if (isset($content->hide_section) && $content->hide_section == 'no')
    @if ($content->style == 'style_3')
        <section
            class="general-content-wrapper pt-20 pb-80 bg-white privacy-policy @if ($content->extra_class) {!! $content->extra_class !!} @endif"
            @if ($content->id) id="{!! $content->id !!}" @endif>
            <div class="container-fluid-lg">
                @if (!empty($content->image))
                    <div class="general-conent-icon fadeText mx-auto w-[60px] h-[60px] lg:w-[80px] lg:h-[80px] xl2:w-[120px] xl2:h-[120px]">
                        <img class="mx-auto lozad img-fluid" src="{!! $content->image['url'] !!}" alt="{!! $content->image['alt'] !!}"
